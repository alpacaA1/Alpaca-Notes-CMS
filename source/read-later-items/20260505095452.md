...
permalink: read-later/20260505095452/
layout: read-later-item
date: 2026-05-05 09:54:52
read_later: true
nav_exclude: true
external_url: https://tw93.fun/2026-05-01/ai-visibility.html
source_name: Tw93
reading_status: unread
tags:
desc: 有小伙伴说我的作品被 AI 主动推荐了，想了想居然啥也没做就被收录了，于是花了一个小时把内容结构化做了一遍。做了什么、哪些有效、哪些别碰。
---

#### Categories: Share 2026-05-01

![AI 可见性封面图](https://gw.alipayobjects.com/zos/k/w5/ai.png?x-oss-process=image/auto-orient,1/resize,w_2000/format,webp)
...
![AI 可见性路线图：从 robots.txt 到主域名镜像的完整配置](https://gw.alipayobjects.com/zos/k/lq/6.png?x-oss-process=image/auto-orient,1/resize,w_2000/format,webp)
...
![AI 爬虫五大分类：训练爬虫、搜索检索、用户触发、退出标识和未声明爬虫](https://gw.alipayobjects.com/zos/k/kq/GjgBCz.png?x-oss-process=image/auto-orient,1/resize,w_2000/format,webp)
...
![llms.txt 文件示例，包含项目概览、链接和内容分区](https://gw.alipayobjects.com/zos/k/3f/4WQmuF.png?x-oss-process=image/auto-orient,1/resize,w_2000/format,webp)
...
![四个站点的 llms.txt 互相引用形成网状结构，AI 爬虫从任意入口发现全部站点](https://gw.alipayobjects.com/zos/k/x4/Rt8NoI.png?x-oss-process=image/auto-orient,1/resize,w_2000/format,webp)

这些改动需要等爬虫重新抓取才会生效，通常要几天。配好之后隔一段时间去 ChatGPT 搜一下自己的项目名，引用来源和描述准确度应该会有变化。

![](https://gw.alipayobjects.com/zos/k/ci/3bugwW.png?x-oss-process=image/auto-orient,1/resize,w_2000/format,webp)
...
![HTML 页面 15000 token 对比 Markdown 3000 token，减少 80% 噪音](https://gw.alipayobjects.com/zos/k/st/AFayJg.png?x-oss-process=image/auto-orient,1/resize,w_2000/format,webp)
...
![Bing Index 驱动 Copilot、DuckDuckGo、Yahoo 和 ChatGPT，通过 IndexNow 即时通知更新](https://gw.alipayobjects.com/zos/k/x3/7.png?x-oss-process=image/auto-orient,1/resize,w_2000/format,webp)
...
![Yobi 知识端点首页，展示项目列表和 API 端点](https://gw.alipayobjects.com/zos/k/e9/ndHtSI.png?x-oss-process=image/auto-orient,1/resize,w_2000/format,webp)

JSON API 返回的结构化数据，包含项目信息和实时 GitHub 统计：

![yobi.tw93.fun 的 JSON API 响应，包含项目结构化数据和实时 GitHub 统计](https://gw.alipayobjects.com/zos/k/an/2RQyzN.png?x-oss-process=image/auto-orient,1/resize,w_2000/format,webp)
...
![Pake 项目知识页面，包含可引用摘要、核心特性、竞品对比和安装命令](https://gw.alipayobjects.com/zos/k/dj/EqrRKi.png?x-oss-process=image/auto-orient,1/resize,w_2000/format,webp)
...
![GitHub Action 每日从 Yobi 子域名同步数据到 tw93.fun 主域名的架构图](https://gw.alipayobjects.com/zos/k/ds/5.png?x-oss-process=image/auto-orient,1/resize,w_2000/format,webp)
...
![GEO 研究：权威引用 +115%，直接引用 +43%，相关统计 +33%](https://gw.alipayobjects.com/zos/k/g5/8.png?x-oss-process=image/auto-orient,1/resize,w_2000/format,webp)
...
![ChatGPT 准确描述开源项目的版本号、功能列表和最新 release](https://gw.alipayobjects.com/zos/k/b4/Ejryss.png?x-oss-process=image/auto-orient,1/resize,w_2000/format,webp)
