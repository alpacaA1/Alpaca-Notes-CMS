  ...
  return <section className="movie-journal" aria-label="光影观影笔记">
    ...
    <section className="movie-journal__detail">
      {!draft ? <div className="movie-journal__detail-empty"><span>◌</span><h2>选择一部电影</h2><p>让感想成为这张卡片的主角。</p></div> : <div className="movie-journal__form">
        ...
        <div className="movie-journal__meta-grid"><label><span className="movie-journal__meta-label"><MetaIcon type="status" />状态</span><select value={draft.status} onChange={(event) => update('status', event.target.value as MovieStatus)}><option value="watched">已看</option><option value="wish">想看</option></select></label><label><span className="movie-journal__meta-label"><MetaIcon type="date" />观影日期</span><input type="date" value={draft.watchedAt} onChange={(event) => update('watchedAt', event.target.value)} /></label><label><span className="movie-journal__meta-label"><MetaIcon type="rating" />个人评分</span><select value={draft.rating ?? ''} onChange={(event) => update('rating', event.target.value ? Number(event.target.value) : null)}><option value="">未评分</option>{[1,2,3,4,5].map((value) => <option key={value} value={value}>{'★'.repeat(value)}</option>)}</select></label><label><span className="movie-journal__meta-label"><MetaIcon type="year" />年份</span><input value={draft.year} onChange={(event) => update('year', event.target.value)} placeholder="2026" /></label><label><span className="movie-journal__meta-label"><MetaIcon type="director" />导演</span><input value={draft.director} onChange={(event) => update('director', event.target.value)} placeholder="导演" /></label><label><span className="movie-journal__meta-label"><MetaIcon type="tags" />标签</span><input value={draft.tags.join('、')} onChange={(event) => update('tags', event.target.value.split(/[、,，]/).map((tag) => tag.trim()).filter(Boolean))} placeholder="孤独、家庭" /></label></div>
