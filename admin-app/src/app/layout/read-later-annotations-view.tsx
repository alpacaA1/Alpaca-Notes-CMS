...
export default function ReadLaterAnnotationsView({
  ...
  const listWheelStateRef = useRef<{ frameId: number | null; deltaX: number; target: HTMLElement | null }>({
    frameId: null,
    deltaX: 0,
    target: null,
  })
  ...
  useEffect(
    () => () => {
      if (listWheelStateRef.current.frameId !== null) {
        window.cancelAnimationFrame(listWheelStateRef.current.frameId)
      }
    },
    [],
  )
  ...
  function handleListShellWheel(event: WheelEvent<HTMLElement>) {
    ...
    const pendingDelta = listWheelStateRef.current.target === currentTarget ? listWheelStateRef.current.deltaX : 0
    const nextScrollLeft = Math.max(0, Math.min(currentTarget.scrollLeft + pendingDelta + event.deltaY, maxScrollLeft))

    if (nextScrollLeft === currentTarget.scrollLeft + pendingDelta) {
      return
    }

    event.preventDefault()
    listWheelStateRef.current.target = currentTarget
    listWheelStateRef.current.deltaX = nextScrollLeft - currentTarget.scrollLeft

    if (listWheelStateRef.current.frameId !== null) {
      return
    }

    listWheelStateRef.current.frameId = window.requestAnimationFrame(() => {
      const { target, deltaX } = listWheelStateRef.current

      if (target && deltaX !== 0) {
        const maxTargetScrollLeft = target.scrollWidth - target.clientWidth
        target.scrollLeft = Math.max(0, Math.min(target.scrollLeft + deltaX, maxTargetScrollLeft))
      }

      listWheelStateRef.current.frameId = null
      listWheelStateRef.current.deltaX = 0
      listWheelStateRef.current.target = null
    })
  }
