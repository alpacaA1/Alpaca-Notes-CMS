  ...
  return <section className="people-book" aria-label="人物簿">
    ...
    <section className="people-book__detail">
      {!draft ? <div className="people-book__detail-empty"><Portrait person={{ name: '人' } as PersonEntry}/><h2>选择一个人</h2><p>记录关系、近况与被文章提起的时刻。</p></div> : <div className="people-book__form">
        ...
        <section className="people-book__moments"><div className="people-book__section-head"><span>日常片段</span><small><PersonIcon type="mention"/>{mentionCounts[draft.id] || 0} 次文章提及</small></div><div className="people-book__moment-composer"><MovieDatePicker value={momentDate} onChange={(val) => setMomentDate(val)} ariaLabel="日常片段日期" dialogLabel="选择日常片段日期" /><textarea value={moment} onChange={(event) => setMoment(event.target.value)} placeholder="今天发生了什么？" /><button type="button" className="people-book__moment-submit-btn" onClick={appendMoment} disabled={!moment.trim()}>记下</button></div>{draft.moments.length ? <div className="people-book__moment-list">{draft.moments.map((item) => <article key={item.id}><time>{item.date}</time><p>{item.content}</p><button type="button" onClick={() => removeMoment(item.id)} aria-label="删除这条日常记录">×</button></article>)}</div> : <p className="people-book__moment-empty">日常会慢慢长成你们的时间线。</p>}</section>
