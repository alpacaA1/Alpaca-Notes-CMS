...
describe('App preview mode', () => {
  ...
  it('shows topic backlinks in a collapsible right drawer when previewing a topic article', async () => {
    ...
    expect(screen.queryByRole('heading', { name: '反向引用' })).toBeNull()
  ...
  it('renders read-later metadata and structured sections in preview mode', async () => {
    ...
    expect(screen.getAllByRole('link', { name: /查看原文/ }).some((link) => link.getAttribute('href') === 'https://example.com/original')).toBe(true)
    expect(screen.queryByRole('img', { name: 'Read-later preview item' })).toBeNull()
