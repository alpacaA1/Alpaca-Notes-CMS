...
describe('App read-later import flow', () => {
  ...
  it('uploads imported remote images and rewrites read-later markdown to site image paths', async () => {
    ...
    expect(image.getAttribute('src')).toMatch(/^https:\/\/alpaca-notes-cms\.vercel\.app\/api\/images\?path=images%2F\d{4}%2F\d{2}%2F\d+-p735699706\.jpg(?:&token=[^&]+)?$/)
  })
 ...
 第二段`.replace(/^ /gm, ''),
  ...
  it.skip('shows RSS load success as a toast without taking layout space', async () => {
    ...
    expect(await screen.findByRole('status')).toBeTruthy()
    expect(screen.getByRole('status').textContent).toContain('已加载《Anthropic News》最近 20 条内容。')
    expect(container.querySelector('.success-message')).toBeNull()
  })
  ...
  it.skip('refreshes RSS items after entering the system so the top-bar badge uses current item URLs', async () => {
    ...
    await waitFor(() => {
      expect(container.querySelector('[aria-label="1 条 folder 待读"]')?.textContent).toBe('1')
    })
  })
